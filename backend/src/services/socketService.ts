...
import { User, ChatMessage, JoinRoomRequest, JoinRoomResponse, SocketEvents } from '../types';
    ...
    io.on('connection', (socket: Socket) => {
        ...
        socket.on('send-message', (data: { content: string }) => {
            try {
                ...
                console.log(`Message sent in room ${currentRoomId} by ${user.nickname}`);
            } catch (error) {
        ...
        socket.on('disconnect', () => {
            try {
                ...
                if (currentUserId && currentRoomId) {
                    ...
                    const user = roomService.removeUserFromRoom(currentRoomId, currentUserId);
