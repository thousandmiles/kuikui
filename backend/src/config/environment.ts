...
export function loadBackendConfig(): BackendConfig {
  ...
  if (isNaN(roomExpiryHours) || roomExpiryHours <= 0) {
    throw new Error(`Invalid ROOM_EXPIRY_HOURS: ${env.ROOM_EXPIRY_HOURS}. Must be a positive number.`);
  }
  ...
  const frontendUrl = env.FRONTEND_URL || `http://${frontendHost}:${frontendPort}`;
